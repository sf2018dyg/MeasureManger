...
//    ImageView rightKuan;
    ...
    public EvaluateRankView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        View view = View.inflate(context, R.layout.view_evaluate_ranking, this);
                    ...
                    mContext.runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            if(list.size()==1){
                                ...
                                rlRank2.setVisibility(VISIBLE);
                                rlRank3.setVisibility(INVISIBLE);
                                tvLeftRanking.setText(getRanksort(list, 1));
                                tvfLeftName.setText(list.get(1).getUserName());
                                tvfLeftGoodRate.setText(list.get(1).getGoodRank() + "%");
                                tvfLeftGrade.setText(list.get(1).getFeedbackAvg() + "分");
                                tvMiddleRanking.setText(getRanksort(list, 0));
                                tvfMiddleName.setText(list.get(1).getUserName());
                                tvfMiddleGoodRate.setText(list.get(0).getGoodRank() + "%");
                                tvfMiddleGrade.setText(list.get(0).getFeedbackAvg() + "分");
                                ImageUtil.loadImageWithCache(mContext, list.get(1).getFheadpic(), leftAvatar, R.drawable.user2, true);
                                ImageUtil.loadImageWithCache(mContext, list.get(0).getFheadpic(), middleAvatar, R.drawable.user2, true);
                            }else if(list.size() >= 3){
                                rlRank1.setVisibility(VISIBLE);
                                rlRank2.setVisibility(VISIBLE);
...
//                                    }
     ...
     * @param view     图片控件
     */
    public void avatarLoading(String imageUrl, ImageView view, int defaultResId) {
        if (imageUrl == null) {
            imageUrl = "";
        }else{
