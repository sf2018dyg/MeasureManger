 ...
 * 类用途：我对客户的评价 - 待评价
 */
public class UnEvaluate_IToCustomersFragment extends
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        etmAdpter = new Evaluate_IToCustomersAdapter(mActivity, 0, itemListener, Evaluate_IToCustomersActivity.REQUEST_CODE_UEITC);
        // 设置布局管理器
            ...
            switch (v.getId()) {
                case R.id.tvfServicesAvailable:
                    ...
                    if (!ectBean.getTasktype().equals("")) {
                        if (Integer.parseInt(ectBean.getTasktype()) == 4) {////主动追踪任务
    ...
    public void RefreshData(boolean isRefresh) {
        ...
        presenter.getEvaluatedList(0, 1);
    }

    @Override
    protected void loadmoredata() {
        if (pageTurnBean != null && pageTurnBean.getPageCount() > pageTurnBean.getPageNo()) {
            presenter.getEvaluatedList(0, pageTurnBean.getPageNo() + 1);
        } else {
            finishRefresh();
    ...
    //显示待评价
    public void showNList(boolean isSuccess, PageTurnBean pt, List<Evaluate_IToCustomersBean> beans, String msg) {
        if (isSuccess) {
            if (pt != null) {
                if (pt.getPageNo() <= 1) {
