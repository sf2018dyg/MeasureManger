package com.soonfor.measuremanager.home.complexruler.fragment;
...
import com.soonfor.measuremanager.home.complexruler.adapter.FuChiListAdapter;
import com.soonfor.measuremanager.home.complexruler.presenter.FuChiBasePresenter;
import com.soonfor.measuremanager.home.complexruler.view.IFuChiBaseView;
 ...
 * 邮箱：suibozhu@139.com
 */
public abstract class BaseFuChiFragment extends BaseFragment<FuChiBasePresenter> implements IFuChiBaseView {

    Unbinder unbinder;
    FuChiMainActivity mActivity;
    @BindView(R.id.recyView)
    RecyclerView mRecyclerView;//1, mRecyclerView2, mRecyclerView3, mRecyclerView4;
    private FuChiListAdapter goAdapter;//1, goAdapter2, goAdapter3, goAdapter4;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mActivity = (FuChiMainActivity) getActivity();
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        goAdapter = new FuChiListAdapter(mActivity, mActivity, gobList, gettype(), listener);
        // 设置布局管理器
    ...
    protected void initPresenter() {
        presenter = new FuChiBasePresenter(this);
    }
...
/*        if (FuChiMainActivity.currPosition != 0) {
            FuChiMainActivity.currPosition = 0;
            RefreshData(true);
        }*/
    }
    ...
    public void refreshAfterAccept(int position) {
        DataTools.ListMapFuChi.get(0).remove(position);
        if (DataTools.ListMapFuChi.get(0)==null || DataTools.ListMapFuChi.get(0).size() == 0) {
            showNoDataHint("暂无数据！");
        } else {
            goAdapter.notifyDataSetChanged(DataTools.ListMapFuChi.get(0));
        }
    ...
    public void setListView(int type, PageTurnBean pt, liangchiHeadBean lhb) {
        ...
        if (DataTools.ListMapFuChi.containsKey(type) && DataTools.ListMapFuChi.get(type) != null) {
            if (pt != null && pt.getPageNo() > 1) {
                totalList = DataTools.ListMapFuChi.get(type);
            }
        }
        totalList.addAll(lhb.getList());
        DataTools.ListMapFuChi.put(type, totalList);
        ...
        public void onClick(View v) {
            ...
            switch (v.getId()) {
                case R.id.llfItem:
                    ...
                    LiangChiBean lb = (LiangChiBean) v.getTag();
                    if (lb != null) {
                        b.putParcelable("LiangChiBean", lb);
                        b.putInt("LiangChiStatus", gettype()+1);
                    }
                    mActivity.startNewAct(mActivity, FuChiDetailActivity.class, b);
                    //startNewAct(FuChiDetailActivity.class, b);
                    break;
                case R.id.tvName:
                ...
                case R.id.jushou:
                    ...
                    ndialog = dialog.getInputDialog(mActivity, "拒收复尺", "" +
                                    "请填写您的拒绝原因!",
                            new OnBtnClickL() {
                                @Override
    ...
    public void showDataToView(String returnJson) {
        ...
        goAdapter.notifyDataSetChanged(DataTools.ListMapFuChi.get(gettype()));
    }
