...
public class LoftListFirstChildAdpter extends BaseAdapter<LoftListFirstChildAdpter.ViewHolder, MarkComponentEntity> implements View.OnClickListener {

    private List<MarkComponentEntity> clist;
    private int viewType;

    public LoftListFirstChildAdpter(Context context, List<MarkComponentEntity> list, int viewType) {
        super(context);
        this.clist = list;
    ...
    @Override
    public LoftListFirstChildAdpter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new ViewHolder(mInflater.inflate(R.layout.adapter_lofting_detail_loftinfo_child, parent, false));
    }

    @Override
    public void onBindViewHolder(LoftListFirstChildAdpter.ViewHolder holder, int position) {
        if (viewType == 2) {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(MarkComponentEntity comBean) {
            tvfMember.setText(comBean.getName());
            String status = comBean.getStatus();
