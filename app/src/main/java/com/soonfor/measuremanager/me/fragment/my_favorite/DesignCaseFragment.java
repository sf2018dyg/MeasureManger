...
public class DesignCaseFragment extends BaseFragment<MyFavoritePresenter> implements IMyFavoriteView{//, SearchView.SearchViewListener{
...
//    SearchView mSearchView;
    ...
    private MycaseListAdapter caseAdpter;
    private List<DesignCaseBean> caseList;
    //String InputText = "";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
       ...
       // mSearchView.setSearchViewListener(this);
        ...
        caseAdpter = new MycaseListAdapter(mActivity, caseList,true);
        // 设置布局管理器
        mRecyclerView.setLayoutManager(mLayoutManager);
        // 设置adapter
        mRecyclerView.setAdapter(caseAdpter);
        caseAdpter.setOnItemClickListener(new MycaseListAdapter.OnRecyclerViewItemClickListener() {
            @Override
            public void onItemClick(View view, int data) {
                DesignCaseBean db = caseList.get(data);
...
//    }
    ...
    public void RefreshData(boolean isRefresh) {
        ...
        if(pageTurnBean.getNextPage()==1){
            caseList = new ArrayList<>();
        }
        presenter.getFavroiteDatas(1,pageTurnBean.getPageNo(), 10, "", isRefresh);
    }

    @Override
    protected void loadmoredata() {
        if (pageTurnBean!=null && pageTurnBean.getPageCount() > pageTurnBean.getNextPage()) {
            presenter.getFavroiteDatas(1, pageTurnBean.getPageNo(), 10, "", false);
        } else {
            finishRefresh();
    ...
    public void setListView(String returnStr) {
        ...
        List<DesignCaseBean> newList = new ArrayList<>();
        if (headBean != null) {
                    ...
                    try {
                        ...
                        if (ja != null && ja.length() > 0) {
                            for(int i=0; i<ja.length(); i++) {
                                DesignCaseBean caseBean = gson.fromJson(ja.get(i).toString(), DesignCaseBean.class);
        ...
        if(caseList==null) caseList = new ArrayList<>();
        ...
        caseAdpter.notifyDataSetChanged(caseList);
    }
    @Override
    ...
    private void addSlidingMenu() {
        ...
        mRecyclerView.setSwipeMenuItemClickListener(new SwipeMenuItemClickListener() {
            ...
            public void onItemClick(SwipeMenuBridge menuBridge) {
                ...
                caseList.remove(adapterPosition);
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        caseAdpter.notifyDataSetChanged(caseList);
                        if(caseList.size()>0) {
                            if (adapterPosition >= 0) {
                                caseAdpter.MoveToPosition(mLayoutManager, mRecyclerView, adapterPosition);
                            }
                        }else {
                            showNoDataHint("暂无热帖");
                        }
