...
public class RichTextEditor extends ScrollView {
    ...
    //private Bitmap bmp;
    //private ArrayList<UploadData> imagePaths;//图片地址集合
    public static Map<String, String> framePics;//上传的视频对应的第一帧图片
    /**
     * 自定义属性
     **/
    //插入的图片显示高度
    ...
    public RichTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        rtTextSize = ta.getInteger(R.styleable.RichTextView_rt_view_text_size, 12);
        ...
        allLayout.setPadding(15, 15, 15, 15);//设置间距，防止生成图片时文字太靠边，不能用margin，否则有黑边
...
//					if(!ss.equals("")){
//						lastFocusEdit.setSelection(ss.length());
//					}
//				}catch (Exception e){}
//
//			}
//		});
    ...
    public void setHint(String hint) {
        if (lastFocusEdit != null) lastFocusEdit.setHint(hint);
    }
...
//					if(imagePaths.get(i).getImagePtth().equals(editData.imagePath)){
//						imagePaths.remove(i);
//						break;
//					}
//				}
...
//				.start(activity);
    ...
    public void getFouse(OnFocusChangeListener focusListener) {
        lastFocusEdit.setHint("内容");
        lastFocusEdit.setOnFocusChangeListener(focusListener);
    }
     ...
     * 插入一张图片
     */
    public void insertImage(Bitmap bitmap, final String fameAtPath, String imagePath) {
        try {
            ...
            if (lastEditStr.length() == 0) {
                ...
                addImageViewAtIndex(lastEditIndex + 1, bitmap, fameAtPath, imagePath);
            } else if (editStr1.length() == 0) {
                //如果光标已经顶在了editText的最前面，则直接插入图片，并且EditText下移即可
                addImageViewAtIndex(lastEditIndex, bitmap, fameAtPath, imagePath);
            ...
            Toast.makeText(activity, "插入内容出错，请重新输入", Toast.LENGTH_SHORT).show();
        }

        hideKeyBoard();
    }

    private boolean isNeedInsert = true;//是否需要插入数据
    ...
    public void insertText(String message) {
        ...
        if (lastEditStr.length() == 0) {
            //如果当前获取焦点的EditText为空，直接在EditText下方插入图片，并且插入空的EditText
            isNeedInsert = false;
            lastFocusEdit.setText(message);
            //lastFocusEdit.setText(message.length());
        } else if (editStr1.length() == 0) {
            isNeedInsert = false;
            //如果光标已经顶在了editText的最前面，则直接插入图片，并且EditText下移即可
            lastFocusEdit.setText(message + lastEditStr);
            //lastFocusEdit.setText((message+lastEditStr).length());
        } else if (editStr2.length() == 0) {
            isNeedInsert = false;
            // 如果光标已经顶在了editText的最末端，则需要添加新的imageView和EditText
            lastFocusEdit.setText(lastEditStr + message);
            //lastFocusEdit.setText((lastEditStr + message).length());
        } else {
            isNeedInsert = false;
            //如果光标已经顶在了editText的最中间，则需要分割字符串，分割成两个EditText，并在两个EditText中间插入图片
            //把光标前面的字符串保留，设置给当前获得焦点的EditText（此为分割出来的第一个EditText）
            lastFocusEdit.setText(editStr1 + message + editStr2);
            //lastFocusEdit.setText((editStr1 + message + editStr2).length());
        }
        hideKeyBoard();
    }
     ...
     * 在特定位置添加ImageView
     */
    public void addImageViewAtIndex(final int index, Bitmap bmp, String fameAtPath, String imagePath) {
        //imagePaths.add(new UploadData(fameAtPath, imagePath));
        ...
        if (bmp != null) {
            ...
            if (fameAtPath != null) {
                if (fameAtPath.toLowerCase().startsWith("http://")) {
                    RequestOptions options = new RequestOptions()
                            .override(Tools.getScreenWidth(activity), rtImageHeight) //指定图片的尺寸 //指定图片的缩放类型为fitCenter （等比例缩放图片，宽或者是高等于ImageView的宽或者是高。）
                            .fitCenter() //指定图片的缩放类型为centerCrop （等比例缩放图片，直到图片的狂高都大于等于ImageView的宽度，然后截取中间的显示。）
                            .skipMemoryCache(true) //跳过内存缓存
                            .diskCacheStrategy(DiskCacheStrategy.RESOURCE); //只缓存最终的图片
                    Glide.with(activity).load(new File(fameAtPath)).apply(options).into(imageView);
                } else {
                    imageView.setMaxHeight(100);
                    Glide.with(activity).load(new File(fameAtPath)).into(imageView);
                }
            } else {
                RequestOptions options = new RequestOptions()
                        .override(Tools.getScreenWidth(activity), rtImageHeight) //指定图片的尺寸 //指定图片的缩放类型为fitCenter （等比例缩放图片，宽或者是高等于ImageView的宽或者是高。）
                        ...
                        .skipMemoryCache(true) //跳过内存缓存
                        .diskCacheStrategy(DiskCacheStrategy.RESOURCE); //只缓存最终的图片
                Glide.with(activity).load(imagePath).apply(options).into(imageView);
            }
        }
        imageView.setAbsolutePath(imagePath);
        imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);//裁剪剧中
        //imageView.setTag(R.id.image_id, fameAtPath);

        // 调整imageView的高度，根据宽度等比获得高度
        RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(
                LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);//TODO 固定图片高度500，考虑自定义属性
...
//		}, 200);
     ...
     * 在特定位置添加ImageView
     */
    public void addImageViewAtIndex(final int index, final String fameAtPath, final String imagePath) {
        //imagePaths.add(new UploadData(fameAtPath, imagePath));
        RelativeLayout imageLayout = createImageLayout();
...
//			imageView.setLayoutParams(lp);
//
//			.with(getContext()).load(imagePath).centerCrop()
...
//		mTransitioner.setDuration(500);
    ...
    public List<EditData> buildEditData() {
        ...
        for (int index = 0; index < num; index++) {
            ...
            if (itemView instanceof EditText) {
                ...
                if (framePics != null && framePics.containsKey(itemData.imagePath)) {
                    itemData.fameAtPath = framePics.get(itemData.imagePath);
                } else {
                    itemData.fameAtPath = "";
                }
    ...
    public class UploadData {
        public String FrameAtPath;//视频的第一帧图片
        public String imagePtth;//图片或视频的路径

        public UploadData(String frameAtPath, String imagePtth) {
            FrameAtPath = frameAtPath;
            this.imagePtth = imagePtth;
        }

        public String getFrameAtPath() {
            return FrameAtPath;
        }

        public String getImagePtth() {
            return imagePtth;
        }
    }
