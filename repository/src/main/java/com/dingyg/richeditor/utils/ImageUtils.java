...
//        Bitmap newBitmap = zoomImage(bitmap, zoomWidth,zoomHeight);
     ...
     * 调用图库选择
     */
    public static void callGallery(Activity mActivity, Set<MimeType> mimeTypes, int num, int requestCode) {
        //调用图库
        Matisse.from(mActivity)
                .choose(mimeTypes)//照片视频全部显示MimeType.allOf()
                .countable(false)//true:选中后显示数字;false:选中后显示对号
                .maxSelectable(num)//最大选择数量为1
                //.addFilter(new GifSizeFilter(320, 320, 5 * Filter.K * Filter.K))
                .gridExpectedSize(mActivity.getResources().getDimensionPixelSize(R.dimen.grid_expected_size))//图片显示表格的大小
                ...
                .captureStrategy(new CaptureStrategy(true, "com.soonfor.repository.lfb_zffileprovider"))//存储到哪里
